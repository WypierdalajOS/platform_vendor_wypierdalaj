...
func init() {
	android.RegisterModuleType("wypierdalaj_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = wypierdalajExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = wypierdalajExpandVariables(ctx, dep_file)
	...
	cmd := wypierdalajExpandVariables(ctx, String(g.properties.Cmd))
