function __print_wypierdalaj_functions_help() {
cat <<EOF
Additional WypierdalajOS functions:
...
- githubremote:    Add git remote for WypierdalajOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Wypierdalaj model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch wypierdalaj_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/wypierdalaj-*.zip | head -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.wypierdalaj.device | grep -q "$WYPIERDALAJ_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $WYPIERDALAJ_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/WypierdalajOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.wypierdalaj.device | grep -q "$WYPIERDALAJ_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.wypierdalaj.device | grep -q "$WYPIERDALAJ_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/wypierdalaj/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $WYPIERDALAJ_FIXUP_COMMON_OUT ]; then
